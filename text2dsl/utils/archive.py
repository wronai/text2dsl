...
class ArchiveManager:
        ...
        try:
            ...
            with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                for file_path in self._collect_files(self.source_dir):
                    ...
                    if include_patterns:
                        if not any(
                            str(arc_name).startswith(p.rstrip("/")) for p in include_patterns
                        ):
                            continue
            ...
            return ExportResult(
                success=True, path=str(output_path), size_bytes=size, files_count=files_count
            )

        except Exception as e:
            return ExportResult(
                success=False, path=str(output_path), size_bytes=0, files_count=0, error=str(e)
            )

    def export_tar(
        self, output_path: Optional[str] = None, compression: str = "gz"
    ) -> ExportResult:
        """
    ...
    def export_files(self, files: List[str], output_path: str, format: str = "zip") -> ExportResult:
        """
...
def create_project_archive(
    source_dir: str, output_path: Optional[str] = None, format: str = "zip"
) -> ExportResult:
    """
